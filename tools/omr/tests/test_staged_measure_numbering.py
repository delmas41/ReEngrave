...
class TestTheBenchmarkArmDoesNotKeepItsOwnCopy(unittest.TestCase):
    """⚠️ ANTI-DRIFT, ported from the superseded duplicate implementation.

    `benchmarks/omr-cleanup-count-2026-09/export_arm.py` once kept a SECOND
    copy of the numbering rule, and its own map-vs-file control went red on
    108 `(part, measure)` pairs the day the exporter's rule changed. A
    re-derived copy is how a cleanup artefact starts sending a human to the
    wrong bars, so the arm must CALL the exporter rather than restate it.

    ⚠️ This assertion is the one thing worth keeping from the parallel
    implementation that this module replaced -- see
    `benchmarks/omr-measure-numbering-2026-09/FINDINGS.md`, whose `max`-width
    rule is exactly the guess `test_staves_that_disagree_about_a_systems_length_refuse_the_scheme`
    exists to refuse.
    """

    def test_the_arm_calls_the_exporters_numbering(self):
        import inspect
        from pathlib import Path as _P
        from tools.omr.staged import export as SX
        src = _P(inspect.getsourcefile(SX)).resolve()
        ...
        self.assertIn("_document_bar_offsets", arm.read_text(),
                      "the arm must call the exporter's numbering, not restate it")
