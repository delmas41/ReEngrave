...
KNOWN_GAPS: dict[str, str] = {
    "wedge": (
        "CLOSED on the export side 2026-09-04 — hairpins reach the file as "
        "`<wedge>` (`export.measure_wedges`), 0 -> 18 against 17 of truth on the "
        "engraved eleven, Tchaikovsky 6 exact at 6/6. ⚠️ THE DETECTION HALF IS "
        "WIDE OPEN AND IS A SCAN PROBLEM, which the engraved corpus cannot show: "
        "hairpin reading F1 is 1.000 there (n=3, exact page truth), and over 11 "
        "SCANNED pages the detector finds **1 hairpin against 198 `<wedge>` of "
        "truth** — Brahms 1 p2 alone encodes 136 and we read one. ⚠️ A hairpin "
        "is a THIN DIAGONAL LINE, which is the shape Phase 4f moved stems and "
        "beams to classical CV for, on the stated grounds that YOLO bounding "
        "boxes are structurally bad at thin lines; hairpins are the third member "
        "of that family and the only one still left to the detector "
        "(`line_detection` has no hairpin path; `staff_detector` mentions them "
        "only to reject them). Two placement errors also remain on the export "
        "side: a hairpin crossing a barline exports as two "
        "(`_merge_arcs_across_barlines` is already generic over arcs), and one "
        "of Mahler's lands on the staff below its own — 3 of its 4 sit in the "
        "band of the staff above, and a hairpin is ALWAYS below its own staff "
        "(8 of 8 in the page truth)."
    ),
    "barline": (
