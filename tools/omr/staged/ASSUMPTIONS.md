...
| D20 | ✅ **CLOSED 2026-09-11.** `gather_key_signature` now asks `key_signature_template` too, once per candidate clef, under a SEPARATE quantity `Q.KEYSIG_TEMPLATE_FIT` (`adjudicate_clef` weighs `Q.KEYSIG_CLEF_FIT` rows, so pooling them would have moved the clef decision), and `adjudicate_key_signature` reads it into GAPS ONLY — the precedence this entry prescribed, inherited rather than re-decided. Measured against a hand-read print truth on 75 staves: locator 16 correct, template 28, **complementary rather than ranked**; combined READING 16/10/49 → 35/15/25 and FILE 33 right → 44. ⚠️ A header-window repair had to land FIRST: eleven crops contained no clef and no signature at all, and this reader answers a confident `fifths: 0` on an empty window — wiring it in alone would have imported eight fabricated key signatures. `benchmarks/omr-keysig-truth-2026-09/FINDINGS.md`. The entry as written follows. | **GATHER reads key signatures with `key_signature_locator` ALONE — the weaker of the two readers this repo has, by a margin measured on the very page the first shadow run used.** `gather.py:982` imports `locate_key_signature`; `key_signature_template` is referenced nowhere under `tools/omr/staged/`, while `transcribe.py:272` imports it and `transcribe.py:1521` records *"the locator reads 2 of 12 staves given the correct clef and this reads 11"* on Beethoven 5 p.1 | it is a READER swap with a MEASURED precedence rule attached, so doing it without the precedence would ship a regression the old path already priced. Not a defect in anything written here — every verdict the locator produced is correctly recorded and correctly reasoned | port the template reader as a SECOND witness with its own `reader=`, and express *gaps only* as a declared precedence in `adjudicate_key_signature`, not as an `if read is None` chain. ⚠️ **It may not simply win**: it is the one source here that can OVER-count, and letting the fuller reading take it gains 1 staff on Beethoven 5 p.2 and 2 on the Pastoral and costs a WRONG reading on WTC I p.17. Evidence and the run that found it: `benchmarks/omr-staged-shadow-2026-09/FINDINGS.md` |
