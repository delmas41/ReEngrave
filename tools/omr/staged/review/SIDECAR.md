...
| `own_box` with `"staff": "other"` | `glyph`, `staff` | **an ADDITION, 2026-09-23 (roadmap 3.4g).** `Q.HUMAN_BOX_VERDICT` = `"owner:other"` on the glyph, `detail.owner_named = null` and NO `twin_on_the_named_staff` — there is no named staff on which a twin could be looked for. Sean, on his own marks: *"'belongs to violin' were about the fact that they belonged to a different staff"* — the point was NOT THIS STAFF and naming the neighbour was incidental. The named form above is unchanged and is still the stronger claim. ⚠️ `other` is the ONE `staff` value that is not a subject and is not refused; every other non-subject still is. |
...
| `owner:other` | **every per-family refusal** in `adjudicators/family_precision.py` AND `adjudicate_notehead_is_not_a_notehead` | `human_other_staff` — the glyph is refused on THIS staff and awarded to none (CLAUDE.md §10: a resolved contest DROPS the loser, it never relocates it). ⚠️ It does NOT reach `adjudicate_glyph_owner`, whose value must be a staff key; `ownership._human_owner` skips it by name. |
