"""Writes this benchmark's FINDINGS.md. Run from the repo root.

The text lives here so the numbers in it can be re-derived beside it; nothing in
the document is computed, it is prose over `out/`.
"""
from __future__ import annotations

from pathlib import Path

TEXT = r"""# A staff that contradicts its own label
...
def main() -> None:
    dest = Path(__file__).resolve().parents[1] / "FINDINGS.md"
