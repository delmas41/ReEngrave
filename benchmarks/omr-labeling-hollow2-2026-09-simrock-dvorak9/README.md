...
| pages used | PDF pages 5, 6, 7 (0-based) |
...
variable is the printing rather than the photograph. Simrock's ink is the
lightest in this set and its lines the thinnest, which is the point: it is the
opposite end of the range from the bled Litolff the first round labelled.

PDF page 4 (printed 183) is left out entirely — it is the page
`benchmarks/omr-scan-e2e-2026-09` scores, and labelling it would train on the
benchmark.
...
hollow notehead**, 4 in total. Scaled to the batch, expect **≈ 19
hollow noteheads**.

**Thin, and the reason is instructive.** Simrock's ink is the lightest in the set, so its beams and its `cresc.` leave counter-sized holes that a bled Litolff print closes — the score inflates on noise here more than anywhere else, which is what forced the band rather than a top-N.
