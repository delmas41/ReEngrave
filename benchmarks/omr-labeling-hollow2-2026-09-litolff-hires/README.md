# Labeling batch: hollow noteheads — Beethoven 5 finale, the high-resolution Litolff scan
...
| | |
|---|---|
| work | Beethoven, Symphony No. 5 in C minor, Op. 67 — movement IV, Allegro |
| edition | Henry Litolff's Verlag, Braunschweig, 1870, plate 2769 |
| publisher | Henry Litolff's Verlag |
| source | IMSLP 575951 (`library/editions/beethoven/symphony-5-op67/beethoven--symphony-5-op67--henry-litolff-s-verlag-1870--imslp575951.pdf`) |
| raster | 5409x7207 jbig2 bitonal @599 ppi |
| pages used | PDF pages 47, 48, 50 (0-based); the finale begins on PDF page 43 |

**The same plates as the first batch, photographed four times larger.** The
first batch read Beethoven 5 from IMSLP 984073 — 2897x3813 at 600 ppi, the
low-resolution scan of these very plates. This is IMSLP 575951: the same
Litolff 1870 setting at 5409x7207. A hollow head whose counter has closed at
one sampling may still be open at another, so *how the same ink is scanned* is
its own axis of appearance, and it is the cheapest one to vary.

**The movement is the finale, not the first, and that was a correction made
mid-flight.** A first attempt took movement I's pages 2, 4 and 6, ranked 51
cells by shortfall, and a look at twelve of them at random found **no hollow
notehead at all** — the *Allegro con brio* is beamed quavers almost throughout,
so its short bars are short for beam reasons and there is nothing of this class
to label. The finale is in 4/4 with the horns, trumpets and trombones holding
whole notes under the strings, and the same look at its pages finds them in
almost every bar. The shortfall ranking is only as good as the music it ranks:
**it finds bars the pipeline got wrong, which coincides with hollow noteheads
only where the page actually prints them.**

PDF page 43 opens the movement and carries its time signature — without a meter
there is no shortfall to rank by — but **no cell is taken from it**.
...
Twelve of these 56 cells were read by eye: **5 of 12 contain at least one
hollow notehead**, 9 in total. Scaled to the batch, expect **≈ 42
hollow noteheads**.

The hits are concentrated in the brass, which hold whole notes under the strings for bars at a time; the string staves in between are beamed quavers and contribute nothing.
...
```bash
python3 -m tools.omr.annotate.server --bench-dir benchmarks/omr-labeling-hollow2-2026-09-litolff-hires
# → http://127.0.0.1:5050
```
...
python3 -m tools.omr.training.verdicts_to_yolo_labels \
    --verdicts-dir benchmarks/omr-labeling-hollow2-2026-09-litolff-hires/verdicts \
    --manifest benchmarks/omr-labeling-hollow2-2026-09-litolff-hires/cells.json \
    --version-name v<n>-2026-09-hollow-litolff-hires --out-root data/user-labeled \
    --labeler sean --description "hollow noteheads, Litolff 1870 Beethoven 5 finale at high scan resolution" \
    --dry-run
```
