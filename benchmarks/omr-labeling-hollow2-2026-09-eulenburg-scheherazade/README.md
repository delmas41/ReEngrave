# Labeling batch: hollow noteheads — Scheherazade, Eulenburg miniature score
...
| | |
|---|---|
| work | Rimsky-Korsakov, Scheherazade, Op. 35 — movement I |
| edition | Ernst Eulenburg, Leipzig, plate E.E. 2957 |
| publisher | Ernst Eulenburg |
| source | IMSLP 1010338 (`library/editions/rimsky-korsakov/scheherazade-op35/rimsky-korsakov--scheherazade-op35--eulenburg--imslp1010338.pdf`) |
| raster | 2938x4293 jbig2 bitonal @600 ppi on a 353x515 pt page |
| pages used | PDF pages 1, 2, 3 (0-based) |

**A miniature score, and that is the point.** Eulenburg's *kleine
Partitur-Ausgabe* prints a full orchestra on a page barely larger than a
paperback, so at the same 600 ppi a staff here is roughly half the height of
one in the Litolff Beethoven. Everything downstream normalises staff span, so
these cells reach the detector **upsampled** where the others arrive at scale
or downsampled — a different appearance of a notehead's counter for a reason
that has nothing to do with ink.

The movement opens **Largo e maestoso** with the Sultan's theme in unison
across the orchestra, written in whole and half notes: the densest
concentration of exactly this class anywhere in the batch set.

PDF page 0 is transcribed for the meter and contributes no cells — it holds the
title block and only a short first system.
...
The whole notes of the *Largo e maestoso* come in stacks — one sampled cell held four — so the box count per hit cell is high even though the hit rate is middling.
...
```bash
python3 -m tools.omr.annotate.server --bench-dir benchmarks/omr-labeling-hollow2-2026-09-eulenburg-scheherazade
# → http://127.0.0.1:5050
```
...
python3 -m tools.omr.training.verdicts_to_yolo_labels \
    --verdicts-dir benchmarks/omr-labeling-hollow2-2026-09-eulenburg-scheherazade/verdicts \
    --manifest benchmarks/omr-labeling-hollow2-2026-09-eulenburg-scheherazade/cells.json \
    --version-name v<n>-2026-09-hollow-eulenburg --out-root data/user-labeled \
    --labeler sean --description "hollow noteheads, Eulenburg miniature Scheherazade" \
    --dry-run
```
