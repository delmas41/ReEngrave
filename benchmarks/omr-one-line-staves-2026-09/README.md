...
| ⚠️ **`line_detection._staff_line_spacing`** | `< 2` → **`24.0`** — and a canonical cell's spacing is `CANONICAL_STAFF_SPAN_PX / 4 = 100`. **4× wrong**, silently: every stem-length gate and beam cluster measured in the wrong unit | **FIXED** — the true canonical spacing is stated on the cell |
