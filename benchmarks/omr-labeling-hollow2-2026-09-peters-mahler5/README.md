# Labeling batch: hollow noteheads — Mahler 5 Adagietto, Edition Peters
...
| | |
|---|---|
| work | Mahler, Symphony No. 5 — movement IV, Adagietto |
| edition | Edition Peters No. 3087b |
| publisher | C. F. Peters |
| source | local scan, 2016, unidentified uploader (`library/editions/mahler/symphony-5/mahler--symphony-5--unidentified-scan-2016--local.pdf`) |
| raster | 4385x5857 jbig2 bitonal @600 ppi |
| pages used | PDF pages 174, 175, 176, 177 (0-based); the movement begins on PDF page 173 |

**The batch the first round dropped for time**, and a fourth house style —
Edition Peters — on a jbig2 bitonal scan.

**The movement is the Adagietto, and that was a correction made mid-flight.**
The obvious choice was the opening *Trauermarsch*, whose sustained writing is
exactly this class. It produced **five cells from four pages**. The cause is
worth writing down because it is structural and not about Mahler: the
Trauermarsch is in **cut common**, and `time_signature_locator` deliberately
does not search for `timeSigCutCommon` — a C with a stroke correlates with any
vertical ink crossing any rounded blob, so enabling it claimed a meter on seven
systems that print none (`benchmarks/omr-timesig-2026-08`). With no meter there
is no shortfall, and with no shortfall this selector has nothing to rank:
**`select_short_bar_cells` is blind on a cut-common movement by construction.**

The Adagietto is in 4/4, which is read, and it is the most sustained music in
the symphony — harp and strings alone, half and whole notes tied across bars
down most of the page.
...
Twelve of these 56 cells were read by eye: **9 of 12 contain at least one
hollow notehead**, 12 in total. Scaled to the batch, expect **≈ 56
hollow noteheads**.

**The richest batch of the five.** The Adagietto is harp and strings alone, sustained through most of its bars, and it shows: three quarters of the sampled cells carry one.
...
```bash
python3 -m tools.omr.annotate.server --bench-dir benchmarks/omr-labeling-hollow2-2026-09-peters-mahler5
# → http://127.0.0.1:5050
```
...
python3 -m tools.omr.training.verdicts_to_yolo_labels \
    --verdicts-dir benchmarks/omr-labeling-hollow2-2026-09-peters-mahler5/verdicts \
    --manifest benchmarks/omr-labeling-hollow2-2026-09-peters-mahler5/cells.json \
    --version-name v<n>-2026-09-hollow-peters-mahler --out-root data/user-labeled \
    --labeler sean --description "hollow noteheads, Edition Peters Mahler 5 Adagietto" \
    --dry-run
```
