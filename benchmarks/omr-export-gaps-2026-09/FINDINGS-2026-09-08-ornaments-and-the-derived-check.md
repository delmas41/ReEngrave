...
| the unit suite | 2,910 pass on clean `HEAD`; **2,974 pass on the final tree**, 2 pre-existing failures — see §6 |
...
`test_score_language.py` was excluded from that run because it is very slow
here — a corpus-wide test over all 1,422 margin labels — and was then run on
its own: **26 passed in 399 s**. ⚠️ Reported separately rather than folded in,
because it was excluded for a reason and a reader should be able to see that
the exclusion was CLOSED rather than left standing.

**So the whole suite on the final tree is 2,974 passed, 64 skipped, 2 failed**,
and the two are the pre-existing `TestReaderSelection` pair.
