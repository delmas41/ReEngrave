#!/bin/bash
# The SAME four pages as `benchmarks/omr-cleanup-count-2026-09`, gathered with
# the margin-label reader actually reachable (`--surya --ocr`).
#
# ⚠️ EXPORT IS A SEPARATE STEP. `staged/__main__.py` imports the exporter AFTER
# the gather, so a mid-run edit reaches it -- and a module imported at process
# START keeps its code, so a mid-run edit silently does NOT. Neither hazard can
# touch a record produced without `--musicxml`.
#
# ⚠️ OMR_SURYA_KEEP_ALIVE=0 so this run OWNS its worker and can repair itself.
# NEVER `pkill -f` anything here: one machine has one shared Surya server.
#
#   bash benchmarks/omr-part-join-phase2-2026-09/run_gather.sh 1-4 labels
set -euo pipefail
cd "$(dirname "$0")/../.."

PAGES="${1:-1-4}"
TAG="${2:-labels}"
OUT="benchmarks/omr-part-join-phase2-2026-09/out"
mkdir -p "$OUT"
