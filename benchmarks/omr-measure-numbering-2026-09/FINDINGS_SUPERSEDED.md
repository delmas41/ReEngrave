⚠️⚠️ **SUPERSEDED — THIS DOCUMENTS A PARALLEL IMPLEMENTATION THAT DID NOT SURVIVE.**

Two sessions built this rule independently, a day apart, neither aware of the
other: `9e2c695c` (2026-09-14, `_document_bar_offsets`) and this one
(2026-09-15, `system_bar_starts`). **`_document_bar_offsets` is what ships**;
`system_bar_starts` and its tests were deleted at the merge.

**The deciding difference is one case, and it is not a style preference.**
Where two staves of one system DISAGREE about how many bars that system prints,
this implementation took the **`max`** — and said so in its own docstring, while
noting the case never arises on the document it was measured on (*"all 75
staff-runs agree"*). `_document_bar_offsets` **REFUSES THE WHOLE FILE** instead,
falling back to the previous per-part numbering and reporting why.

⚠️ **Refusing is right by this repo's own doctrine**: taking the `max` converts
*"the staves cannot agree how long this system is"* into a definite bar number,
which is the conversion `A fallback must never turn "cannot tell" into a
definite answer` forbids. The surviving module's
`test_staves_that_disagree_about_a_systems_length_refuse_the_scheme` and
`test_a_MAJORITY_is_not_taken_it_is_refused` assert exactly what this one's
`test_the_width_is_a_MAX_so_the_ranges_cannot_OVERLAP` asserted the opposite of.
⚠️ **The `max` was never exercised** — that is why two measurements of the same
document agree to the bar and the implementations still differ where it counts.

**What was KEPT from this work**, because the other implementation had no
equivalent: the anti-drift test that
`benchmarks/omr-cleanup-count-2026-09/export_arm.py` must CALL the exporter's
numbering rather than restate it — the second copy of the rule that sent that
arm's own control red on 108 `(part, measure)` pairs. It now lives in
`tools/omr/tests/test_staged_measure_numbering.py`.

**The measurements below stand** and were reproduced independently by the
surviving session; only the implementation is superseded. Kept rather than
deleted, because a superseded measurement beside its correction is worth more
than a gap.
