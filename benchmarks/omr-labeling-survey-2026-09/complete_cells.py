"""Completion detector for the hollow-notehead gate (round 2).

For each round-2 batch cell that carries a human hollow box, run the PRODUCTION
detector (per-cell imgsz rule, conf>=0.50) to recover the NON-hollow content
(black noteheads, rests, accidentals, aug dots, flags) so training does not treat
those symbols as background. Filters + dedupes, drops anything overlapping a human
hollow box, and writes:
  ...
  <survey>/completion_summary.json            aggregate counts

NOTHING here writes to verdicts/ (human record). The audited/merged verdicts are
produced by a separate step after this one's output is reviewed.
"""
...
# Completion scope (gate-appropriate): keep only the density-prior-critical
# classes the completion actually needs — BLACK noteheads + augmentation dots.
# Model rests/accidentals/flags are FP-prone on this domain (restWhole/restHalf
# fire on bled slur arcs, accidentalDoubleSharp on trill lines) and add no
# needed positive signal (those classes are already richly covered by v1-v4 +
# DSv2), so they are DROPPED and documented as unlabeled background — the same
# scoped-completeness tradeoff round-1's AUDIT.md recorded. Hollow noteheads are
# HUMAN-labeled, so the model's are dropped too.
...
# Systematic FP culls found in the visual audit (see GATE_RESULTS.md):
#  - a bass/F-clef's two dots read as augmentationDot: they sit in the clef/
#    key-sig zone (cx < 18% of the cell), where a real dot — which trails a
#    note — never falls. Cull augmentationDot there.
CLEF_DOT_CX_FRAC = 0.18
#  - one black notehead fired on a trill wavy line (brahms1-p3-sys0-s8-m3,
#    top-right, well above the staff). Explicit cull by cell + rough position.
EXPLICIT_CULL = [
    # (cell_id, class, cx_min, cx_max, cy_min, cy_max)  in canonical px
    ("brahms1-p3-sys0-s8-m3", "noteheadBlackOnLine", 1850, 2000, 180, 320),
]
...
def not_lr_edge(d, W, tol=8) -> bool:
    """Drop boxes that TOUCH the left/right image edge — in a barline-cut
    canonical cell those are adjacent-measure slivers / barline ink, not this
    measure's noteheads. (Top/bottom fragments are handled separately by
...
def cross_class_nms(dets, iou_thr=0.45):
    """Drop lower-conf det when two kept boxes overlap heavily (any class)."""
...
def main():
    batches = sorted(glob.glob("benchmarks/omr-labeling-hollow2-2026-09-*"))
    detector = YoloDetector(WEIGHTS, device=DEVICE)
    root = Path.cwd()
    summary = {"weights": WEIGHTS, "conf": CONF, "batches": {}}
    grand_kept = {}
    grand_cells = 0
    for b in batches:
        ...
        for vf in vfiles:
            ...
            # drop model boxes that land on a human hollow head
            ...
            # audit-driven systematic FP culls
            ...
            for hb in hboxes:
                ...
                cv2.rectangle(canvas, (x,y), (x+w,y+h), (0,200,0), 4)   # green = human hollow
            for d in filt:
                x,y,w,h = d.x_canonical, d.y_canonical, d.width_canonical, d.height_canonical
                cv2.rectangle(canvas, (x,y), (x+w,y+h), (0,0,255), 3)   # red = model kept
    ...
    Path("benchmarks/omr-labeling-survey-2026-09/completion_summary.json").write_text(json.dumps(summary, indent=2))
    print("="*90)
