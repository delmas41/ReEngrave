...
- **Clef coverage is ~23% on 19th-century prints, though reading is reliable.** Where a clef is read it is essentially always right (7/7 on the hand-checked ground-truth page, 0 false positives on 10 pages of Bach piano), and sampled misses do have a visible clef, so this is lost recall not correct abstention. Coverage is measured by `benchmarks/omr-clef-geometry/probe_clef_rejection.py`, which reports one rejecting branch per header cell over a whole sample. **The oversized-cluster branch holds the rest** — the clef fuses into a cluster bigger than any C clef and the locator stops. Two halves of that are now fixed: the header window fixed the horizontal (widths are clef-sized, median 2.6 spaces), and running the ink-fraction test BEFORE the size test fixed *sparse* blockers — brace residue that x-clustering had drawn one tall box around, which stopped the search in front of a perfectly good clef (Nottebohm: located 32 → 43, oversized 113 → 73). What remains is genuine vertical fusion, and it is now overwhelmingly an ORCHESTRAL problem: Beethoven 5 is still 76% oversized at a height median of 7.2 spaces against a 5-space limit, where Nottebohm is down to 38%. Scoped in `benchmarks/omr-clef-geometry/NEXT_SESSION_HEADER_CLUSTER.md`, which also lists three approaches already measured and closed. See `benchmarks/omr-clef-geometry/RESULTS.md`.
