...
- **Clef coverage is ~17% on 19th-century prints, though reading is reliable.** Measured on a 20-page sample: clefs read on 36 of 188 music staves (19.1%), 23 of them by the CV locator (C clefs the detector cannot see at any confidence). Where a clef is read it is essentially always right (7/7 on the hand-checked ground-truth page), and sampled misses do have a visible clef, so this is lost recall not correct abstention. **55% of header cells fail for one reason** — the clef fuses into an oversized cluster and the locator stops. The header window fixed the horizontal half (widths are now clef-sized, median 2.5 staff spaces); what remains is vertical fusion with the system brace, at 6-9.6 staff spaces tall against a 5-space limit. Scoped in `benchmarks/omr-clef-geometry/NEXT_SESSION_HEADER_CLUSTER.md`. See `benchmarks/omr-clef-geometry/RESULTS.md`.
