...
- **Body text can be detected as staves on mixed text/music pages.** Row ink-count passes the line-length test on justified paragraphs, so a text block becomes a "staff" (147 of 1522 "staves" over 156 pages of Nottebohm). Diagnosed with a measured discriminator (staff span vs page median) but NOT fixed — Phase 1 has no working regression baseline (`test_pipeline.py`'s count assertions already fail from older drift, identically on `main`). See NOTES.md → "Staff detection on mixed text/music pages".
