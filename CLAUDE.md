...
- **Clef coverage is ~17% on 19th-century prints, though reading is reliable.** Measured on a 20-page sample: clefs read on 31 of 188 music staves, 18 of them by the CV locator (C clefs the detector cannot see at any confidence). Where a clef is read it is essentially always right (7/7 on the hand-checked ground-truth page), and sampled misses do have a visible clef, so this is lost recall not correct abstention. **56% of all staves fail for one reason** — the clef fuses with the brace or the key signature into an oversized cluster and the locator stops. Splitting that cluster is the single biggest lever. See `benchmarks/omr-clef-geometry/RESULTS.md`.
