...
- **Clef coverage is ~31% on 19th-century prints, though reading is reliable.** Where a clef is read it is essentially always right, and sampled misses do have a visible clef, so this is lost recall not correct abstention. Coverage is measured by `benchmarks/omr-clef-geometry/probe_clef_rejection.py` (one rejecting branch per header cell over a whole sample); precision by `benchmarks/omr-clef-geometry/check_clef_precision.py`. **Run both — never one alone.** **The oversized-cluster branch holds the rest** — the clef fuses into a cluster bigger than any C clef and the locator stops. Three halves of that are now fixed: the header window fixed the horizontal (widths are clef-sized, median 2.6 spaces); running the ink-fraction test BEFORE the size test fixed *sparse* blockers, brace residue that x-clustering had drawn one tall box around; and grouping header ink by proximity in BOTH axes stopped the movement heading and the rehearsal letter above the staff being strung into the clef's cluster (Nottebohm: located 32 → 43 → 59 of 191 headers; Beethoven 5: 12 → 14 of 371). The vertical rule applies only to ink standing *clear* of the staff — ink that touches it belongs to whatever glyph is printed there, and without that restriction the rule takes a scanned treble clef apart at the waist and reads its upper half as an alto clef. What remains is overwhelmingly an ORCHESTRAL problem: Beethoven 5 is still 46% oversized at a height median of 6.9 spaces against a 5-space limit, where Nottebohm is down to 25%. Scoped in `benchmarks/omr-clef-geometry/NEXT_SESSION_HEADER_CLUSTER.md`, which also lists eight approaches already measured and closed. See `benchmarks/omr-clef-geometry/RESULTS.md`.
