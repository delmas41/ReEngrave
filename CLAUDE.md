...
only in which earlier page is in the window: the TRUE meter scores **+6.0**
(7 bars fit / 2 not) and the FALSE one **−8.0** (0 / 9) — where on the
...
floor — **so the block on a second publisher is READING, not the weighing.**
⚠️ The one engraved false positive is a **CAUTIONARY signature** after a line's
final barline, shown by BOTH printings, which `_meter_changes` has no notion
of. ⚠️⚠️ A third fixture (`C` → `¢`, both 4.0 quarter notes) observes **the
...
| `OMR_METER_SEGMENTS` | `0` off (default) → staged pipeline only: the exporter reads the meter in force at each BAR out of `Q.METER`'s `segments`, so a printed mid-system meter change can reach a file at all. Engraved 4 printed / 4 found / 1 → 0 false; scan unchanged (its false segments are misreads, not misplacements). See the knobs table. |
