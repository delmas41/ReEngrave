...
| `OMR_ENGRAVED_KEYSIG` | `0` off (default) → staged pipeline only: on a document MEASURED engraved, read the key-signature TEMPLATE before the locator. Right **26 → 47** of 50, wrong **24 → 3**; bars exact **313 → 322**, 4 parts better and 0 worse; of 29 quantities exactly 2 move. One-sided — a scan, an abstention, no identity row and the flag off all fall through unchanged. n = 1 engraved document, no print consulted. See the knobs table. |
...
on that info."* Two flags: `OMR_DOCUMENT_IDENTITY` **default ON**,
`OMR_ENGRAVED_KEYSIG` **default OFF**. Findings:
...
`reach.KNOWN_GAPS` entry stays, corrected; no OMR-NED, deliberately. **The
default is Sean's.**
