...
the remaining route is an ENGRAVED render. ⚠️ The hunt also found a bug: the
meter's fallbacks were chained with `or`, and `_carry_meter` returns a truthy
`Ruling` when it is REFUSED — so **a refusal blocked every rung behind it**,
including a meter change printed on that same system (that half predates
`OMR_METER_FROM_BARS`).
