...
- **Body text can be detected as staves on mixed text/music pages.** Row ink-count passes the line-length test on justified paragraphs, so a text block becomes a "staff" (147 of 1522 "staves" over 156 pages of Nottebohm). Diagnosed with a measured discriminator (staff span vs page median) but not yet applied. It was blocked on Phase 1 having no regression baseline; that blocker is cleared — `test_pipeline.py`'s four failing assertions were wrong expectations, now corrected against the pages, with an open-score case added (539 pass, 0 fail). See NOTES.md → "Staff detection on mixed text/music pages".
