...
⚠️ **`C89`** (*An instrument's HEADER CLEF is a property of the instrument…*)
was written straight into this registry on 2026-09-23 (roadmap 2.10) rather
than compiled out of `docs/conventions/from-this-repo.md`, which is NOT
re-counted for it. The Conservation ledger's arithmetic names it separately
for that reason — a registry that silently grew its source file's count would
be claiming a measurement that file does not hold. (Both lanes numbered
...
- **Status:** MEASURED HERE (the slot); the *discriminator between whole and half* is ASSERTED — and as of ROADMAP 2.12b it is READ but still DECIDES NOTHING. ⚠️ Being read is not being measured: no print check has adjudicated the 134 + 433 rows it fires on, which is why the rule narrows and abstains instead of flipping a value, and why this line must not be promoted to MEASURED HERE until Sean has the crops.
...
- **Code:** `tools/omr/staged/adjudicators/rhythm.py` `WHOLE_REST_STEP` (behind `OMR_WHOLE_REST_INK`, default ON) and — **ROADMAP 2.12b, 2026-09-23** — `HALF_REST_STEP` (DERIVED from it, never typed), `REST_SLOT_SLACK = 0.5`, `_rest_slot`, `_rest_slot_verdict` and `_rest_ruling`, which is the discriminator's **first consumer**: it measures the slot with the same `_staff_step` the mirror rule uses, NARROWS over both values where the rectangle lands on the other convention (134 rows on the two scans) and ABSTAINS `rest_stands_where_no_rest_hangs` where it lands on neither (433). ⚠️ It still **decides nothing from the slot** — see Status. `Q.REST_POSITION` at `tools/omr/staged/positions.py:303` remains **PRODUCER ONLY** and is deliberately not read: it files zero rows on all three acceptance records (`OMR_FAMILY_POSITIONS` is default OFF) and `inventory`'s producer AST walks `gather.py` alone, so declaring it opens a finding rather than closing one. `benchmarks/omr-shape-role-2026-09/FINDINGS.md` §2.12b.
...
- **Known exceptions:** ⚠️⚠️ **INK ALREADY CLAIMED BY A CLEF BOX IS OUTSIDE THIS ENTRY, and the measurement put that exception here.** On Brahms 1 p.6 `staff/6/1/3` the detector reads `clefG` at confidence 0.688 on the very ink a first build of the rule then took to `tenor` — a staff that already READ correctly, changed. The complaint is *a **notehead** box on clef ink is too small to be what the ink is*; where the box on that ink is itself a clef, which clef it is belongs to the clef contest and not to a geometric override. The two populations separate EXACTLY rather than by a threshold: both staves this entry exists for (`staff/3/0/9` Litolff p.3, `staff/6/0/9` Breitkopf p.6) carry `Q.CLEF_GLYPH` ABSTAINED `no_detections`. ⚠️ a **treble** clef never reaches this test at all — it is ~7 spaces and `ClefLocatorConfig.max_height_spaces` stops the search first, so this entry is about the **C-clef family only** and the roadmap's prose guess of a treble band is unmeasurable from this reader. ⚠️ A **mid-staff clef change** stands past the header and is out of scope — `Q.CLEF` is staff-scoped and no arm reads past cell 0. ⚠️ A box that is ITSELF inside the size band does not count as "too small" and keeps its veto.
- **Code:** `tools/omr/clef_locator.py` — `ClefLocatorConfig.clef_family_min_height_spaces` / `_max_height_spaces`, `_overridable_occupancy`, `locate_clef`'s `occupied_classes`; `tools/omr/staged/gather.py` `_occupied_notehead_rows` and `_clef_boxes` and `gather_clef_locator`; consumed by `tools/omr/staged/adjudicators/notehead_precision.py` `_is_a_clef` (refusal `is_a_clef`).
...
for"*, and C89 is ASSERTED with a measured reach of ZERO. Adding a row to it
would put an unmeasured claim inside the half of the corpus whose whole value
is that every entry carries a number. It is listed below under its own heading
instead, so the ledger still accounts for every id an entry carries.
