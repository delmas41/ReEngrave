...
⚠️⚠️ **AND THE EXPERIMENT BOTH 09-16 HANDOFFS NAME AS "the one that closes it"
— the per-system arm on `p0p3` — WAS RUN ON 2026-09-15 AND IS ON MAIN**, cited
by neither of them nor by this file until 2026-09-16 (found by `git log`, the
§7 pattern a fourth time): [benchmarks/omr-meter-carry-brahms-2026-09/FINDINGS.md](benchmarks/omr-meter-carry-brahms-2026-09/FINDINGS.md)
— the carry was ASKED on 5 of 7 systems and REFUSED on all 5, and the four
arms (OFF / CARRY / BARS / BOTH) export **byte-identically** (one md5 ×4),
because `_meter_in_force_at_end` hands on the later `4/4` segment and never
the voted `9/4` — **the misread does not propagate, structurally.** ⚠️ It
measured a pre-`A-METER-6` tree, and that rule only makes a carry MORE
conservative, so the direction is a prediction on current main, not a
re-measurement. ⚠️ Both handoffs also list the flag as *"still off"*; **it has
been ON since 09-15** (`rhythm.py:1016`, a deny-list). Corrected at every site.
...
(2026-09-17, `Q.INK` / `OMR_INK`, **default ON since 2026-09-17**, PRODUCER ONLY) AND THE FRAME
...
| `OMR_DIRECTION_TEXT`  | `1` (on) | **On by default since 2026-09-02.** Reads the words printed inside a system — `legato`, `Allegro con brio` — by subtracting every detection from the page's ink, refusing the curves by fill ratio, OCRing what is left with Surya and Tesseract, and gating on a lexicon of musical terms. Emitted as MusicXML `<words>`. **Worth 144 edits** on the engraved orchestral benchmark, 18.8% of the pooled figure, and `wrong direction` is the third-largest bucket. **Additive** — every word placed reaches the file and the export is identical outside its `<direction>` blocks, checked per page on engravings and on a scan. Costs 0.5-0.8 s per candidate crop; the ~70 s model load it used to be blamed for belongs to the margin-label reader, which is on by default and loads Surya first on any page without a text layer. Self-disables where neither `.venv-surya` nor Tesseract exists. CLI: `--no-direction-text`. See `benchmarks/omr-direction-text-2026-09/DEFAULT_2026-09-02.md`. ⚠️⚠️ **REPRICED ON A SCAN 2026-09-16 AND IT HAD NEVER BEEN: on the STAGED path it is the most expensive thing in a run and returns almost nothing there.** Litolff Beethoven 5 p1-4, six ABAB arms: **~267 s/page**, against **93 s/page** for the margin-label rungs it shares a model with — and the `LD` arm (labels on, directions OFF) is **faster than every arm including those carrying no label reader at all**. What it buys there, by diffing two exports that differ only in this flag: **six `<words>`, and all six are `cresc.` in three casings** (`CRESC.` x3, `Cresc.` x2, `cresc.`) — with those and the `<direction>` wrappers they emptied removed, the two files are **BYTE-IDENTICAL** (notes 2459, rests 851, dynamics 205, slurs 80, ties 177 all unmoved). **~178 seconds per word.** ⚠️ **NOT an argument for turning it off**: the 144 engraved edits are untouched and unre-measured. The shape of the fix is a DOMAIN GATE, and the machinery already exists inside this very reader — `default_readers` already drops the Tesseract rung where `page_is_engraved` PROVES a page is vector, and that classifier is `OMR_WEIGHT_ROUTING`'s, measured, and answers False on any doubt. Unbuilt, unmeasured, and a DEFAULT, so Sean's call. n = 1 document; a Breitkopf scan fires 371 flag boxes to this one's 49, so a text-richer scan could pay for itself. See [benchmarks/omr-surya-staged-cost-2026-09/FINDINGS.md](benchmarks/omr-surya-staged-cost-2026-09/FINDINGS.md). |
...
| `OMR_FAMILY_POSITIONS` | `0` (off) | **Staged pipeline only, PRODUCER ONLY.** One staff-relative POSITION row per mark, for the eleven families `capture.py` graded `position: NONE`. Sean, 2026-09-17: *"give the families real position information — or if it should be symbol specific then make it so"*, and *"Theoretically every position can at some point contribute even if that is not with our current pipeline."* ⚠️ **A SEPARATE ROW FROM A SEPARATE READER, carrying NO confidence** — a ruler reading is not a guess, and `Q.CLEF_POSITION`'s docstring records why a detail field on the glyph row CANNOT work: `Evidence.correlated_groups` calls every row from one reader on one crop ONE SIGNAL, so a position hung off the glyph row is absorbed by that glyph's own detector term and is not a second witness. ⚠️ **NOTHING READS THEM, deliberately** — the `Q.INK` discipline: a producer and its first consumer landing together makes the reach measurement circular. Each one's first consumer is NAMED in `reach.KNOWN_GAPS`, and **an entry must LEAVE the day its consumer lands**, because *unread and unremarked* is how `Q.STEM` stayed unread through three separate discoveries. ⚠️ Allow-list ON test, correct for a default-OFF flag. See `tools/omr/staged/positions.py` and [benchmarks/omr-family-positions-2026-09/FINDINGS.md](benchmarks/omr-family-positions-2026-09/FINDINGS.md). |
| `OMR_HOLD_OUT_UNIDENTIFIED` | **`1` (on)** | **Staged pipeline only. A staff the join could not NAME is held OUT of the file instead of being emitted as a part of its own.** Sean's call 2026-09-17: *"hold out - I want truth"*. ⚠️⚠️ **EMITTING A PART IS A POSITIVE CLAIM, AND THAT WAS THE BUG**: a `<part>` says *"here is an instrument"*, while what we know of an unnamed staff is *"this music belongs to a part we could not name"* — CANNOT TELL converted into a definite answer, and on Litolff Beethoven 5 pp.1-4 it invented **25 instruments** on top of the 12 the page prints, so a human opening the cleanup artefact could not read it. **Parts 37 → 12, measures 1727 → 1332.** ⚠️ **THE MUSIC IS COUNTED, NEVER SWALLOWED**: 783 notes are dropped under `staff_not_identified` and the accounting control stays an EQUALITY, so the shortfall is visible rather than dressed as an orchestra. ⚠️ **SCOPED TO THE `slot` JOIN**: on the `fragments` fallback nothing is named, so holding out the unidentified would write an EMPTY FILE — a rule that turns a bad page into no page. ⚠️⚠️ **IT GOVERNS THE OUTPUT, NOT THE ENQUIRY, AND THE FIRST VERSION CONFLATED THOSE** — the staff still exits identification after ONE channel (`adjudicate_instrument` abstains `no_evidence` on an empty margin), which is a separate open fault recorded under *A PREMISE ENCODED IN A REFUSAL OUTLIVES ITS REASON*. ⚠️ Deny-list OFF test, default-ON. `0` restores the pre-2026-09-17 exporter exactly. |
...
| `OMR_SLOT_CONSTRAINTS` | **`1` (on)** | **Staged pipeline only. THE THREE CHANNELS SEAN NAMED, AS CONSTRAINTS ON THE ORDER-PRESERVING MAP.** Sean, 2026-09-22: *"margin names, instrumentation lists from the doc or a dossier, Order, family brackets should all come first in determining what the instrument is"* — and the clef only where we are sure of it, because *"the clef is a small part of deciding what the instrument is and we would need to be sure of the clef to have it impact the instrument."* `adjudicate_slot_index` enumerates every strictly-increasing staff→slot map (`[C62]`) that (a) puts each NAMED staff on a slot of its own name, (b) puts each staff in its system's Nth family block into the reference's Nth (`[C58]`, `Q.STAFF_GROUP` — where the interior barlines STOP, **not** a printed bracket, which Litolff has none of), and (c) puts each UNNAMED staff on a slot whose instrument may be printed in the clef GATHER read there. A staff takes a slot only where EVERY surviving map agrees. ⚠️ **REACH, measured before a line was written** ([benchmarks/omr-instrument-channels-2026-09/FINDINGS.md](benchmarks/omr-instrument-channels-2026-09/FINDINGS.md)), over the staves whose instrument ABSTAINED, **ZERO GRAFTS in every arm on both documents**: order alone **0 of 25** on Litolff and **38 of 40** on Breitkopf; + family block **5**; + clef **18**. ⚠️⚠️ **ORDER ALONE FORCES NOTHING ON THE DOCUMENT THAT NEEDS IT** — Litolff's unnamed staves are a TRAILING block with no named neighbour below, so the whole block slides — **and Breitkopf INVERTS it**, its unnamed staves being interleaved among named ones. *Neither document could have shown that alone.* ⚠️⚠️ **"A SLOT'S CLEF IS WHAT THE REFERENCE SYSTEM READ" IS REFUTED**: a slot's clef is constant on both plates for every instrument **except the Cello and the Bassoon**, which alternate bass/tenor as a matter of course — Breitkopf's reference reads its cello slot `tenor` and three later systems read the same slot `bass`, **all four at margin 4.5** — and that premise makes **5 of Breitkopf's 6 systems unsatisfiable**, discarding the clef channel on every one. Taking each slot's admissible clefs from the **instrument's own lexicon entry** (`ALTERNATING_CLEFS`) reaches the same 18 and is refused on **no system of either document**. ⚠️ **A CLEF READING ON A NAMED STAFF CAN ONLY SUBTRACT** — the name already pins it, and on Litolff p2/s1 the Fagotti's FALSE `tenor` (the CV locator unopposed at exactly `MARGIN_FLOOR`) names a clef no slot admits, the system goes unsatisfiable and **four forced staves are lost to one wrong reading on a staff the clef was never needed for**; the channel speaks about UNNAMED staves only. ⚠️ **A CHANNEL MAY NARROW OR ABSTAIN AND MAY NEVER EMPTY THE SOLUTION SET**: where one makes a system unsatisfiable it is DROPPED for that system and the fact recorded, weakest first — Sean's *evidence contributes, it never gates*, made mechanical. ⚠️ **IT FILTERS `_place_in_family_block`, IT DOES NOT REPLACE IT**: the narrowing keeps its reason and detail so `inferences.collapse_slot_index_to_family_block` still reads them. ⚠️⚠️ **THE FIRST CUT DECIDED DIRECTLY AND COST TWO STAVES THROUGH A RULE DOWNSTREAM OF IT** — deciding two members of a block removed them from `_block_members`, which requires the narrowings to cover `0..k-1`, so INFER skipped the block whole and two Violas the shipped path places correctly came out narrowed. **A branch additive in its own terms can still subtract through a later rule, and only a PER-STAFF base-vs-arm control shows it.** Repaired in `_block_members` (a member the page FORCED is not a hole; it is admitted so the block reconstructs and is never proposed for, since `INFERABLE` is `{NARROWED, ABSTAINED}`). **MEASURED**, one gather decided four ways on ONE tree, control **75 of 75** committed `slot_index` verdicts reproduced with both branches off: placements are **IDENTICAL to the shipped path** — 25 correct, 5 narrowed, **0 grafts, 0 staves moved** — and the provenance moves, `forced_by_constraints` **0 → 13** against INFER's `the_short_block_is_condensed_at_its_foot` **15 → 2**. ⚠️ **THE COUNT DOES NOT MOVE AND MUST NOT BE QUOTED AS A GAIN**: what changes is that 13 placements are ENTAILED by the page rather than assumed by a convention. ⚠️ **All 12 parts were already NAMED before this** (`name_arm.py`, every arm) — `export._default_name`'s `Staff p1-s0-3` does **not** appear on this record, so the symptom `docs/NEXT-2026-09-22-instrument-identification.md` §5 describes is not present here and this change does not address it. ⚠️ **NOT ESTABLISHED**: no print was consulted by this session (correctness rests on the 2026-09-14 and 2026-09-17 crop passes); Breitkopf is **unscored** (`printed-lineups.json` is LITOLFF ONLY and an arm that scored a Brahms record against it once reported 21 grafts that were its own); no export, no file, **no OMR-NED** (musicdiff does not score `<part-name>`); n = 2 documents, 2 publishers, 8 pages, **both scans**, engraved untouched. Deny-list OFF test, correct for a default-ON flag. `0` restores the pre-2026-09-22 behaviour exactly. |
...
wide.
⚠️⚠️ **VERIFIED BY AN INDEPENDENT PASS 2026-09-15, AND IT IS NOW THE ONE STAGED
REPAIR BEHIND A FLAG** (`OMR_WHOLE_REST_INK`, default ON = the behaviour that
shipped; see the knobs table). Re-derived from the shipped constants over the
same record — md5 checked against the artefact's own — the fire set is
**identical to `adjudicated-25-fires.json` subject for subject with zero
measurement drift**, the witnesses re-derive (**148 / 310 / 25** of 2,347), and
all 25 were **re-cropped from the PDF by a second instrument: 25 of 25 are a
filled rectangle at the whole-rest slot, not one oval notehead.** ⚠️ **The
flag exists because the rule DELETES music on n = 1**, not because anything
measured against it. ⚠️ Two ledger defects found in the same pass, neither
behavioural: the commit message says the crops *"are committed under `crops/`"*
and **`.gitignore:112` has excluded `benchmarks/**/crops/` all along**, so they
were never in the tree; and `adjudicate_notehead_is_a_whole_rest`'s comment
says a `Q.REST` row carries no page box, while **all 646 rest rows in the
record carry `detail.bbox_page_px`** and `gather.py`'s own emit site passes it.
The code reads `Q.GLYPH_BOX` and is correct either way — but *the tree outranks
the ledger*, including a ledger written as a commit message inside it.
...
cell's largest component holds 46% of its ink*. ⚠️⚠️ **THAT FIGURE WAS ONE CELL AND IT IS NOW
MEASURED, AND THE TYPICAL COMPONENT IS STEM-SHAPED AFTER ALL.** Over **2,001
cells of both plates** (0 drift) the median component is **4.18 / 3.20 staff
spaces**, only **6%** of components are past the width cap, genuine merges
**across notes** are **9.6% / 12.0%**, and **~94% of the over-cap mass covers
NO notehead at all** — it is the furniture the cap exists for. So *"the ink is
one blob"* is **true of the extreme and false of the median**: the fusion is
real, it is not the common case, and the diagnosis stands for a different
reason — a component reader cannot separate the stem from the notehead it is
joined to, whatever the rest of the distribution looks like. This paragraph
carried the 8.1-space figure with a one-cell caveat for one day before it was
measured; the caveat was the right one to have written.
...
`OMR_WHOLE_REST_INK` asks for exactly this — *"Breitkopf Brahms 1 p0-3 is where
it should be re-measured"* — and the answer is that its **outcome** survives and
its **stated safety case does not**. Findings and probes:
`benchmarks/omr-second-publisher-pricing-2026-09/`, adjudication row-by-row in
`adjudicated-12-fires.json`. Record md5 verified against the receipt
(`52b98f1cdfee3b39f10e56c592828ea4`); `rhythm.py` and `export.py` are
byte-identical between the measured tree and `origin/main`, checked.

⚠️ **THE PROBE IS THE DECISION, ESTABLISHED IN BOTH DIRECTIONS.** Run over the
Litolff record it reproduces the shipped derivation to the third decimal
(`0.460 / 0.840 / 1.631 / 3.087`) and **25 fires at `slot 20 / neighbour 5`**,
the published figures; run over Breitkopf its **12 fires are subject-for-subject
identical to the 12 `notehead_is_a_whole_rest: true` verdicts the record already
holds**. Percentiles use the shipped `v[int(f*len(v))]`, never `numpy`.

⚠️⚠️ **THE BAND DESCRIBES 89.9% OF LITOLFF'S OWN WHOLE RESTS AND 52.5% OF
BREITKOPF'S** — the false-negative side as a pure number, over a population the
rule never judges, with near-identical denominators (395 vs 398). Two of the
three shape cuts move in OPPOSITE directions: max height p95 **0.840 → 0.670**
(25% too loose here) and max aspect p95 **3.087 → 4.633** (33% too tight,
refusing **42.5%** of this plate's real whole rests). Only `MIN_ASPECT`
transfers (1.631 → 1.615).

⚠️⚠️ **AND THE POSITION WITNESS INVERTS, WHICH NO CONSTANT SWEEP COULD HAVE
FOUND.** The NEIGHBOUR witness rests on *a tacet part prints a whole rest in
every bar* — a claim about rests INSIDE their own staff. **18.3% of Breitkopf's
`restWhole` detections stand outside their own staff against Litolff's 3.5%,
and 64 of them sit more than a space BELOW it where Litolff has ZERO**; step
p05/p95 goes `2.47 / 5.93` → **`−7.53 / 6.28`**. So the firing path flips from
`slot 20 / neighbour 5` to **`slot 1 / neighbour 11`**, and **11 of 12 fires
stand outside their own staff against 3 of 25.** It is the *correlated
witnesses* hazard with the correlation running through **the measure-cell crop**
— a third door onto that room, after ink and convention.

⚠️⚠️ **ALL TWELVE CROPPED AT THE GATHER'S OWN 600 DPI, BEHIND A FRAME CONTROL
THAT CAN FAIL (it passed at +133 to +229 grey levels): ONE IS A WHOLE REST.**
Eight are the upper hook of an **eighth rest on the staff below**, two are
**real noteheads** on a neighbouring staff, one is the top arc of the printed
**`6` of the `6/8`**. Against Litolff's **25 of 25**. ⚠️ **The record
corroborates the hand verdicts on 8 of 12 from an independent direction** — by
page-pixel overlap with no class matching, the staff below reads `restWhole`
(IoU 0.57), `rest8th` ×6, and `noteheadHalfInSpace` (IoU 0.69) on the same ink.

⚠️⚠️ **THE ONE REAL NOTE AMONG THE FIRES IS SAVED BY A DIFFERENT MECHANISM.**
`glyph/1/1/5/7/11` is a printed hollow notehead with an augmentation dot;
**both of this rule's own witnesses accepted it**, and only the 2026-09-11
ownership contest (`owned_by_another_staff`) keeps it in the file. So *"zero
real notes deleted"* is true here and **is not this rule's doing** — wherever
the contest does not fire, nothing measured here would stop it.

**MEASURED, one record exported twice** (`whole_rest_export_arm.py`): pitched
`<note>` **2,524 → 2,513**, **11 removed / 0 added**, `ink_is_a_whole_rest`
0 → 12, `owned_by_another_staff` 294 → 293 — **12 = 11 + 1 to the unit**. ⚠️ The
structural control FAILED and its residue is the evidence: stripping every
`<note>` leaves **one `<backup>`**, a bar that stopped being two-voice, which
`two_voice_bars 71 → 70` says from the other side. **The strip was NOT widened
until it passed.** ⚠️ The 11 removed are all SPURIOUS readings, so **the rule's
outcome is good here and its reason is wrong eleven times in twelve.**

⚠️⚠️ **A RE-EXPORT IS THE RIGHT INSTRUMENT AND THAT NEEDED AN ARGUMENT, NOT A
SHORTCUT.** A plain re-export is normally BLIND to an ADJUDICATE change, and
this is one — but `grep -rn NOTEHEAD_IS_A_WHOLE_REST tools/ --include='*.py'`
returns the declaration, the producer, the `ORDER` entry and **exactly one
reader, `export.py:542`**, with the flag read at EXPORT time. **4 seconds
instead of ~2.5 hours** of re-adjudicating 443 MB twice. The day anything reads
that quantity inside ADJUDICATE, the arm stops being valid.

⚠️ **THE FALSE-NEGATIVE SIDE IS POPULATED AND NOBODY HAD LOOKED.** Of 248
non-fires POSITION already vouches for, ranked by fractional excess over the cut
they failed, **the closest is a REAL WHOLE REST at the slot** — its box clips
the rest's left half so the aspect reads **1.51** against the 1.63 floor, and
the record exports it as an **eighth note C5**. Sean's original observation,
un-repaired on this publisher, inside the population the repair was built for.

⚠️ **TWO HARDENINGS, PRICED ON BOTH HAND-ADJUDICATED POPULATIONS AND OFFERED AS
A MEASUREMENT RATHER THAN A RECOMMENDATION.** *The glyph must stand INSIDE its
own staff* takes Litolff **25 → 22 fires (22 of 22 still whole rests)** and
Breitkopf **12 → 1**; *slot witness only* gives 20 and 1. It needs no new
constant and would make the rule's claim true on 22 of 23 fires across both
documents instead of 26 of 37 — ⚠️ **at the measured cost of 3 correct Litolff
catches and 11 useful Breitkopf deletions put back.** That trade needs a human,
and it needs a third publisher more.

⚠️ **RECOMMENDATION, NOT A CHANGE: leave the flag ON, and stop quoting
*"25 of 25, zero real notes"* as its safety case** — that is a Litolff figure.
The transferable statement is weaker: *it removes ink that should not have been
written as a note, usually for the wrong reason.* **What is NOT established**:
that it is wrong to be on; that a third publisher behaves like either of these
(n = 2 documents, 7 pages); anything about the ENGRAVED family; and no OMR-NED
figure is claimed, deliberately — the metric is symmetric and would pay for
these deletions whether or not they were right.

⚠️⚠️ **THE DOTTED REST IS ANSWERED ON THE SAME RECORD AND IT IS A CLEAN
NEGATIVE.** `benchmarks/omr-staged-dotted-rest-2026-09/FINDINGS.md` moved ZERO
verdicts on Litolff, blamed reach (35 `aug_dot` rows), and named this document,
which fires **656**. ⚠️ **It needed no arm** — the repair is already in the tree
the record was gathered on, so the record's own `duration` verdicts carry it.
**Breitkopf: 656 dots, 1,129 decided rest durations, exactly ONE dotted rest.
Litolff: 35 dots, 738 rest durations, ZERO.** ⚠️ The **454 dotted NOTES of
2,828 (16.1%)** are the positive control that the dot reader is working hard
here, so the `1` is a real answer and not a dead reader. ⚠️⚠️ **And that one
firing is a FALSE dot: it is the SMALLEST `aug_dot` of all 656** (width 0.180
staff spaces against a median of 0.490, rank 1 of 656) and the crop shows a
clean UNDOTTED whole rest in a tacet bar. **So 691 `aug_dot` rows across two
publishers attach to a rest exactly once, and that once is wrong** — the
adjudicator's *"848 rows attach 752 to a notehead and exactly ONE to a rest"*,
independently confirmed at 33× the dot density. ⚠️ **Not an argument for
reverting it**: a consistent gap is worse than a consistent rule with no reach.
The finding is that **its reach is ~0 on both publishers measured**, so no work
should be ranked by it.
...
### ⚠️⚠️ THE PRICING ARM RAN — its "REACH IS ZERO" is REFUTED (5 of 7), and the per-system arm had run the DAY BEFORE
...
measurement.
⚠️⚠️ **AND THAT ARM TOO HAD ALREADY BEEN RUN — 2026-09-15, on main since
`328c7ac0`, cited by nothing in this thread until 2026-09-16.**
[benchmarks/omr-meter-carry-brahms-2026-09/FINDINGS.md](benchmarks/omr-meter-carry-brahms-2026-09/FINDINGS.md)
is exactly the four-arm re-adjudication over the shared Breitkopf record the
paragraph above asks for: the carry asked on **5 of 7**, refused on **5 of 5**,
exports **byte-identical ×4**, blast radius **zero** (0 `<type>`, 0 pitch, 0
rest moves), and the candidate in hand was **`4/4`, not `9/4`** — the
`OMR_METER_SEGMENTS` repair is what stops the exact propagation the brief
feared. Its own §10 says the objection is **neither retired nor confirmed**:
the page refuses a correct candidate as readily as a wrong one. ⚠️ So the
sentence *"the cost side is unmeasured, on a document where the carry IS asked
with a wrong candidate in hand"* is half wrong: it IS measured there, at zero,
because the bars refuse everything. What is unmeasured is a page whose bars CAN
speak while a voted misread is in force — the `m7p0p3` boundary fixture
(carried `C` refused at −8.0, bars naming 3.0) is the only observation of that
shape and it is two systems. Three sessions named this run as the next
experiment after it existed; **`git log --all -S carry-brahms` before
dispatching the fourth.**
...
⚠️ **Corrected 2026-09-16: the propagation question on this document was
answered on 09-15** — [benchmarks/omr-meter-carry-brahms-2026-09/FINDINGS.md](benchmarks/omr-meter-carry-brahms-2026-09/FINDINGS.md), carry asked 5 of 7, refused 5, byte-identical;
see the section above. The open question is the FORM-borrowing rule, below.
...
is `local_arm.py`, on Sean's machine, never run. ⚠️ **Overtaken: it ran on
both documents on 09-15/16** (Breitkopf: carry asked 5 of 7, refused 5,
byte-identical; Litolff p1-3: 1 of 5 → 5 of 5, 298 wrong→exact / 0 the other
way) — see the `OMR_METER_CARRY` row and [benchmarks/omr-meter-carry-brahms-2026-09/FINDINGS.md](benchmarks/omr-meter-carry-brahms-2026-09/FINDINGS.md).
...
original mtime**, so a `.pyc` written during one arm still satisfies Python's
`(mtime, size)` validity check during a later one — and that arm then imports
the **UNMUTATED** code and reports **NOT RED**. Two arms were affected and one
was hand-debugged as a missing assertion before the cause was found, while the
same mutation applied by hand OUTSIDE the battery went red immediately.
`PYTHONDONTWRITEBYTECODE=1` in the arm's subprocess environment took that
battery from **10 RED / 2 survived to 12 RED / 0**. ⚠️ So the prophylactics are
all three — commit a checkpoint first, write an in-flight sentinel, and disable
bytecode in the arms — **plus VERIFY BY HASH that each mutation actually changed
the file**, because an anchor that matches once and a write that succeeds are
...
⚠️⚠️ **AND THE RESIDUAL IS NOT A FAULT EITHER — TWO STEMS ARE READ AS ONE.**
Three of the 26 unexplained cases were cropped and shown: *"they look like
clean notes with basic stems on them"*, and then Sean diagnosed it: *"there are
two separate stems that are being measured as one stem."* ⚠️⚠️ **The decisive
test is an asymmetry a single stem CANNOT produce — a stem STARTS at its head,
so one stem can overshoot its note at ONE end only. Flagged runs overshoot at
BOTH ends on 7 of 14 Breitkopf cases (50%) and 9 of 51 Litolff**, the worst
reaching 2.5 noteheads above the top head and 2.8 below the bottom over 5.7
spaces — the crop showing two voices, upper stemming UP to the beam above and
lower stemming DOWN to the beam below, fused into one component. Three
measurements agree: flagged runs are LONGER (5.72 vs 4.12; 4.43 vs 3.46),
leave a SUBSTANTIAL piece on BOTH sides when cut at the flagged head (2.75 /
3.06 and 2.50 / 2.03), and are **~18% WIDER** (two stems at slightly different
x). ⚠️ An earlier guess in this section — *"merged with a staff line"* — is
WITHDRAWN: it was a story with no test.

> **The convention is sound and the notes are ordinary. TWO STEMS ARE READ AS
> ONE VERTICAL RUN, and the note that appears stranded is the one at the
...
vertical runs are not segmented correctly.**
...
The ranked next work of the `OMR_VERTICAL_RUNS` row above, taken.

⚠️⚠️ **THE RESULT, AND IT INVERTS A CLEAN NEGATIVE.** That lane measured Sean's
rule — *"bar lines are the length of a staff or they extend to other systems"* —
as **0 of 19 print-settled barlines firing at any tolerance while 6 of 58
adjudicated STEMS did**, *"faintly backwards"*, and then diagnosed it: the runs
are measured inside a MEASURE CELL, whose reach is the staff plus 4 + 4 staff
spaces, so a barline taller than that has its ends clipped BY the crop. Read off
the **whole page** instead, with **nothing else moved** — same adjudicated
population, same tolerance sweep, same two forms of the rule:
...
**Between 0.25 and 0.60 spaces the rule fires on 11-13 of 19 barlines and on
ZERO of 63 stems.** ⚠️⚠️ **THE DECISIVE COLUMN IS THE STEM ONE, because the
barline half inherits the predecessor's circularity** — the crop pass sampled
its barlines FROM the `too TALL` bucket, so *"these marks are tall"* is true by
construction. The stem half is not circular (sampled across all six buckets),
and its false-positive side is EMPTY across four tolerances.

⚠️ **THE NARROW FORM IS STILL 0 OF 19 IN BOTH FRAMES**, same selection effect,
so **nothing here speaks for a ONE-STAFF barline** — and a sibling lane prices
how much that costs: `omr-barline-height-2026-09` counts **7 of 82** Litolff
barlines and **4 of 49** Breitkopf ones crossing every inter-staff gap, so the
kind measured here is roughly **8% of the barlines on the page.** That lane's
own §4 names this reader in advance.

⚠️ **NOT A BAND — THE PAGE, and the difference is Sean's own rule.** A per-staff
band deep enough for a one-staff barline still clips a SYSTEMIC one, which
reproduces the fault one window further out. **Nothing is filtered** either
(`Q.INK`'s discipline): `detect_stems` refuses 64.7% of its candidates by a
DIMENSION bound and a barline is refused by `RUN_TOO_TALL` — *a height bound
named for stems, applied to the population that contains barlines.* The opening
recipe is IMPORTED from `detect_stems`, pinned by test, so the two readers
cannot drift about what a vertical run IS while differing about where to look.

⚠️ **AND THE CELL POPULATION WAS DOUBLE-COUNTED, WHICH ONLY A PAGE FRAME CAN
SAY.** 6,128 cell candidates against **3,465 page runs** on the same four
Litolff pages (6,816 against 4,536 on Breitkopf) — cells overlap by 4 + 4
spaces, so a mark in the gap is a candidate in BOTH and a tall one in every cell
it crosses. **This is the first count of vertical marks on these pages that is a
count of MARKS.** It also reports a quantity a cell frame cannot express at all:
**how many staves a run spans** — 16 Litolff runs at 11-12 and 7 Breitkopf at
13-14, landing on exactly those pages' system sizes.

⚠️⚠️ **THE MUTATION BATTERY FOUND THREE CONTROLS SITTING AT THEIR CEILING, AND
THAT IS THE PROCESS FINDING.** Its first run had 4 survivors and only ONE was a
test gap. The other three were controls that cannot fail on a page where the
repair WORKS: the cross-reader rate is 1,920 of 1,920, the cell signature is 0,
and the DEAD exit is never taken — so mutating each changed nothing. **A control
can only be mutation-tested in a state where it FAILS**, so the arm gained a
second positive control (`--clip-like-a-cell`, the predecessor's window
re-imposed), the cross-reader control moved AHEAD of the DEAD exit (its only
failing state is the blinded one), and the battery judges every arm on all three
runs. ⚠️ **And the decisive control was testing for something that CANNOT HAPPEN
here**: the ±4.00 signature is a PAIR, and the pair is an artefact of per-cell
DUPLICATION — a page reader emits the mark ONCE, so even with the cell's window
re-imposed only the far end is cut (measured: `d_bot` returns to exactly 4.00,
`d_top` stays at the ink). Testing for the pair read **zero under the fault as
well as under the repair**. Widened to EITHER end: **0 under the repair, 48 of
48 under the reproduced fault.** Final battery **17 arms, 17 RED, 0 survivors,
restore md5-verified.**

⚠️ **WHAT IS NOT ESTABLISHED: nothing is NAMED.** The reader returns vertical
ink with coordinates and a length; it does not say barline, stem or bracket, and
**no adjudicator reads it.** The test firing says the FACT is available, not that
anything uses it. n = 2 documents / 2 publishers / 8 pages, both SCANS; no
record changed; no OMR-NED, deliberately. ⚠️ The page frame is **6.5× coarser**
than the canonical cell frame (16 page px per staff space on Litolff against
~100 canonical) — the cross-reader control says it is not blind there (100% on
both), and says nothing about box precision. See
[benchmarks/omr-vertical-runs-page-2026-09/FINDINGS.md](benchmarks/omr-vertical-runs-page-2026-09/FINDINGS.md).

---

## The reference sheet has no accidentals in it — a justification, re-run

2026-09-20, `OMR_STEM_NOTEHEAD_GATE`, default OFF. The sibling lane
`benchmarks/omr-stem-pair-rule-2026-09` measured that `_drop_paired_strokes`
is wrong about its own premise and proposed the notehead gate; its §4 recorded
that the gate *"has never been run end to end"*. It has now — and re-running
the fixtures that justified the rule is worth more than the gate. Findings:
[benchmarks/omr-stem-notehead-gate-2026-09/FINDINGS.md](benchmarks/omr-stem-notehead-gate-2026-09/FINDINGS.md).

⚠️ **PHASE 0 FIRST, AND EVERY FIGURE RE-DERIVED TO THE UNIT** before a line
was written: 2,164 candidates → 1,920 shipped, **244** deleted, **164** on no
notehead, **80** carrying one, **62 = 77.5%**, **73** heads (9.2% of 793), and
that lane's structural control 783 of 783 cells matching the committed record
EXACTLY. Nothing differed, which is not the usual outcome in this repo.

⚠️⚠️ **THE RULE'S FIRST CITED FIXTURE PRINTS NO ACCIDENTAL AT ALL.**
`_drop_paired_strokes`' docstring justifies itself on two measurements, and
the first is the LilyPond reference sheet (*"from +7/+8/+5/+2 to −1/0/+1/0"*
on a truth of 48). `benchmarks/omr-phase4-lines/reference-lines.ly` is plain C
major and G major — `c'4 d'4 e'4 f'4`, `<c' e' g'>4`, `g2 b2` — **not one
sharp, flat or natural in 48 stems.** So that win was never a measurement of
accidental rejection, and nobody had asked what it WAS a measurement of.

⚠️⚠️ **IT IS ONE BAR, AND THE PAGE TOTAL IS TWO ERRORS CANCELLING.** All **28
of 28** deletions across the four thicknesses fall in ONE cell of staff 0, and
28 of 28 stand on a detected notehead — which is why the gated arm is
IDENTICAL to the rule-off arm there (page |error| 3 → 27 → 27). At thickness 4
the page cuts **6 cells per staff against the sheet's 6 bars**, so the per-bar
truth `ground-truth.json` has carried all along IS usable
(`probe_per_bar.py` **REFUSES the other three by name** — the caveat
`score_pdf` states, checked rather than inherited). Read there:

> **staff 0 bar 5, "four three-note chords", truth 4: ON 0, OFF 4, GATED 4.**
> **THE SHIPPED RULE EMPTIES THE BAR.** Per-bar summed |error| ON **7**, OFF
> **3**, GATED **3** — the opposite ordering to the page total, which reads
> the other way only because the page over-detects by 3 elsewhere and the rule
> deletes 4 chord strokes, netting −1 against 48.

**A whole-page total cannot see a bar emptied and a bar inflated at once.**

⚠️⚠️ **AND GATED 4 AGAINST A TRUTH OF 4 IS A COINCIDENCE OF COMPOSITION, which
the probe prints itself so the count cannot be quoted without it.** The bar's
four strokes stand at x 2.29 / 2.73 / 11.15 / 11.58 staff spaces in a
19.7-space bar holding four chords — **two x-clusters, not four**: two chords
contribute two strokes each (one 6.0-6.2 spaces, consistent with a stem fused
through its own stacked noteheads, and one 3.13-3.14 beside it) and **two
chords contribute nothing**. The supportable statement is *strokes on 2 of the
...
**+80 and +152** strokes every one on a notehead, **0 lost**, heads with no
stem **793 → 720** and **1,529 → 1,351**. ⚠️ The **−73 reproduces the sibling
lane's 9.2% from the other direction** — they joined the record, this counts
heads with no overlapping stroke on a fresh cut — and the Breitkopf 1,529
matches `rejection_census`'s published figure. On the hand cells it costs
**+5**, all over-counting.

⚠️ **THE PROPOSED FORM WAS NOT THE BEST FORM OF ITSELF, and a unit test found
it, not review.** `probe_proposed_gate.py` also excludes an on-head stroke
from being a PARTNER; an accidental's stroke beside a real stem then loses its
only partner and SURVIVES as a false stem. A stroke on a head is never dropped
either way, so the 62 are safe in both — what the partner rule decides is the
OTHER case, and subject-only is the form that leaves the shipped rule doing
its job. ⚠️ The test's own first fixture was wrong and is recorded at its
site: the notehead box covered BOTH strokes, so the partner met the head too
and a correct gate failed the test. *A fixture that does not isolate the case
tests the fixture.*

⚠️ **WHAT IS NOT ESTABLISHED**: accuracy — **no crop was cut**, and not one of
the 80, the 152 or the 3 hand cells the gate makes worse has been checked
against the print; every accuracy statement is agreement between two of our
own readings, which §5 shows can agree and both be wrong about the same ink.
No export, no file, no OMR-NED (the metric is symmetric and would pay for
emitting fewer strokes either way). n = 2 scanned documents, 2 publishers, 4
pages each; the per-bar result is **1 thickness of 4**; `_resolve_cells` drops
4 of the 14 hand cells on today's tree; and **~90% of the 793 is still
unexplained**, exactly as the sibling lane left it.

**RECOMMENDATION, NOT A CHANGE: leave the flag OFF, and treat the reference
sheet's per-bar behaviour as the item for a human.** A rule in production is
justified by a page total on a sheet that does not contain the thing the rule
names, and the same fixture at its own per-bar resolution says the rule empties
a bar of chords. **The flip is Sean's.**
...
2026-09-17, `Q.INK` + `gather_ink` behind `OMR_INK`, **default ON since
2026-09-17 (Sean's call, deny-list)** — this line read *default OFF* until
2026-09-17 evening — PRODUCER ONLY. Sean's standing request since 2026-09-09, and **the frame changed twice
...
| `OMR_DIRECTION_TEXT_SCAN_GATE` | `0` off (default) → **staged pipeline only**: skip the direction-word reader on a page PROVED to be a scan, where it measured **~267 s/page for six `<words>`** (all `cresc.`, three casings) against 93 s/page for the margin-label rungs it shares a model with. ⚠️ Gates on `direction_text.page_is_scanned`, a POSITIVE proof — *not* `not page_is_engraved`, whose negation is also true of a hybrid, a blank page and an unopenable PDF. Over 216 pages: **0 of 36 engraved pages would be skipped**, 174 of 180 scans gated, the 6 abstentions keeping the reader ON. Abstains `OUT_OF_SCOPE`, never `READER_UNAVAILABLE` — the rungs are installed, we declined to spend them. The legacy `transcribe` path is untouched and its 144 engraved edits are not re-measured. See [benchmarks/omr-surya-staged-cost-2026-09/FINDINGS.md](benchmarks/omr-surya-staged-cost-2026-09/FINDINGS.md). |
...
| `OMR_FAMILY_POSITIONS` | `0` off (default) → staged pipeline only, PRODUCER ONLY: one staff-relative position row per mark for the eleven families that had none, as a SEPARATE scoreless row from a separate reader (a detail field on the glyph row is absorbed by that glyph's own detector term and is not a second witness). Nothing reads them yet, deliberately. See the knobs table. |
