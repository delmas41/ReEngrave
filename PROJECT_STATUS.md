...
`Q.METER` is refused, so the rule never reads the meter it would otherwise
need. ⚠️ **The claim transfers between publishers and the refusals do not** —
on Breitkopf the dominant stop is DISAGREEMENT (96 of 239 against Litolff's
15) on a plate reading 22% of durations right against 59%, so the unanimity
requirement makes the rule **go quiet rather than go wrong**, observed rather
than asserted. **`COLUMN_MIN_INDEPENDENT_WITNESSES` is no longer unmeasured**:
the mass sits at exactly ONE independent group on both documents and both
rules, so the floor at 2 is a boundary and not a midpoint. ⚠️⚠️ **It also
found the stage's own self-check silently circular since 2026-09-15** and
repaired it, with the consequence that the check now correctly REFUSES under
current defaults. ⚠️ Accuracy is NOT established — no inferred note checked
against a print, and no export arm ran.
[docs/handoff-2026-09-17-infer-rule-two.md](docs/handoff-2026-09-17-infer-rule-two.md).
Previously 2026-09-16 (docs reconciled) — the meter-carry ledger corrected against the tree: `OMR_METER_CARRY` is ON (both 09-16 handoffs said *"still off"*), the per-system Breitkopf arm they name as the next experiment ran on 09-15 (carry asked 5 of 7, refused 5, byte-identical — `benchmarks/omr-meter-carry-brahms-2026-09/`), and PR #43's 5-of-7 domain correction is merged; the open decision is the form-borrowing rule. Previously 2026-09-16 (later) — **both OCR rungs now default ON in the staged pipeline** (Sean's call, after the pre-registered experiment passed every clause: 17.8 s/page attributable, L−C 20.5 % of C, 50 of 50 labels correct); one Surya worker per RUN (17 s → 5 s); and the unasked finding — `OMR_DIRECTION_TEXT` costs ~267 s/page for six words on the Litolff scan, repriced but still default ON. [benchmarks/omr-surya-staged-cost-2026-09/FINDINGS.md](benchmarks/omr-surya-staged-cost-2026-09/FINDINGS.md). Earlier the same day: the staged Surya opt-in scoped ([docs/scope-surya-staged-optin-2026-09-16.md](docs/scope-surya-staged-optin-2026-09-16.md)): the "~75% of a whole-work run" it rested on was never measured anywhere, the one same-pages pair says +18% (n=1), Surya is already spawned once per staged page by the direction reader, and the staged record cannot yet tell an absent OCR rung from a page that prints no label — that fix comes before any default flip; see version_memory. Previously: 2026-09-09 — the meter boundary MEASURED on engraved fixtures and on a second document and publisher, then the scan side opened: half its damage was BOOKKEEPING rather than reading (false meter changes 10 → 3, the engraved arms now clean), and `Q.METER`'s `segments` found to reach no file at all; before that, four parallel sessions merged; the **metric is no longer the organising goal** and the staged pipeline (GATHER · ADJUDICATE · EVALUATE) is, see the September 8–9 entry below and [CLAUDE.md](CLAUDE.md)'s START HERE pointer. ⚠️ The arc list below is a running history and the entries between 2026-09-03 and 2026-09-08 were never written into this file — read [version_memory.md](version_memory.md) for that week. Previously: 2026-09-02 (two parallel arcs merged: the overnight generalization session — the engraved benchmark widened 3 → 10 works and opened at twice the incumbent error rate, a five-row SCAN benchmark now exists, the cut-common meter bug and two key-signature vote bugs fixed, see [docs/overnight-2026-09-01-summary.md](docs/overnight-2026-09-01-summary.md) — and the day queue's export-gap arc (fermatas, printed accidentals, the coverage check) plus the branch audit, per [docs/branch-assessments-2026-09-02.md](docs/branch-assessments-2026-09-02.md); the current accuracy figure lives in [CLAUDE.md](CLAUDE.md)'s OMR-NED section, is generated from `benchmarks/omr-ned-2026-08/current-accuracy.json`, and the suite goes red if the two disagree)
...
- **September 16 — the staged Surya opt-in scoped, and the number it rested on was a phantom.** Asked whether to opt Surya in, what it costs per page, and whether to train an in-house OCR. Read off committed artefacts only (no arm run): the "~75% of a whole-work run" cited by the staged CLI and CLAUDE.md exists nowhere; the one same-pages `run_gather.sh` pair is +282 s over 4 pages (+17.8%, n=1, different trees); the direction-text reader already spawns Surya on every staged page, so `--surya` saves one of two spawns; Tesseract-alone and a reordered cascade are both traps (a matched-wrong label is the graft, and the quality merge leaves no early return to save a spawn); and the staged record stamps every label `TEXT_LAYER`, so it cannot tell a venv-less machine from an unlabelled page. Decision: flip both OCR flags together after that record fix and a `--check`-gated ABAB run with pre-registered rules — Sean's call. Do not train an OCR. [docs/scope-surya-staged-optin-2026-09-16.md](docs/scope-surya-staged-optin-2026-09-16.md).

- **September 16 (later) — measured, and switched on.** The §9 experiment ran on Sean's machine with the machine taken exclusively: Part A 17.8 / 17.9 s per page attributable (two runs), Part B L − C 20.5 % of C (drift-corrected), 50 labels over 75 staves and **50 of 50 correct** against hand-read print truth — every pre-registered clause met, so `--surya` / `--ocr` are now **default ON** (`--no-surya` / `--no-ocr` to opt out; Sean's call). Three more things landed with it: the record can now tell an absent OCR rung (`out_of_scope`) from an unlabelled staff (`no_ink`); one Surya worker per RUN instead of per call, 17 s → 5 s measured; and the finding nobody asked for — `OMR_DIRECTION_TEXT`, on by default since 09-02, costs **~267 s/page for six words** on this scan, ~178 s per word, repriced in CLAUDE.md and left ON pending a scan-side decision. Not established: the ~300 s residue, the every-arm-slower drift, anything beyond n=1 document. [benchmarks/omr-surya-staged-cost-2026-09/FINDINGS.md](benchmarks/omr-surya-staged-cost-2026-09/FINDINGS.md).
